function [v, op_sigma, K1, trainingdata, subClassMean, subClassLabels]=KSDA_train_Ram_Maxhomo4(trainingdata,C,nc)
...
for ii=1:numSubClass,
    ...
    [Sigma(ii),fval(ii)] = fminunc(@(sigma)Maxhomo4(H,label,sub_label, C, A, sigma,DD),X0,options);
end
[F,ind]=min(fval);
op_H=ind;
op_sigma=Sigma(ind);
